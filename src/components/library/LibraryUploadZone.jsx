...
const LibraryUploadZone = ({ onFileSelect, uploadFile, isUploading }) => {
  return (
    <div className="md:col-span-12 lg:col-span-4 h-full">
      <div className="bg-white rounded-xl border-4 border-dashed border-primary-container/30 p-10 h-full flex flex-col items-center justify-center text-center group hover:border-primary/50 transition-all cursor-pointer relative overflow-hidden group">
        <input 
          type="file" 
        ...
        <h3 className="text-2xl font-black text-on-surface mb-3 italic uppercase tracking-tighter">Manifest Protocols</h3>
        <p className="text-[10px] text-on-surface-variant font-label font-bold uppercase tracking-widest opacity-60 mb-8 max-w-[200px]">
          Drop your files here <br/> (PDF, JPG, PNG & DOCX up to 100MB)
        </p>
        
        <button className="bg-surface-container-highest text-primary font-label font-black py-4 px-10 rounded-full hover:bg-primary hover:text-white transition-all shadow-sm text-[10px] uppercase tracking-widest relative z-30">
          Browse Files
        </button>

        <div className="mt-10 pt-10 border-t border-outline-variant/10 w-full grid grid-cols-3 gap-4 relative z-30">
          <button className="flex flex-col items-center gap-2 p-3 hover:bg-surface-container-low rounded-xl transition-all group/btn">
